...
class TestProcessor:
    ...
    def test_append_predictions(self) -> None:
        ...
        assert updated["TICKER1"].iloc[-1]["Price"] == pytest.approx(105.0)
        assert updated["TICKER1"].iloc[-1]["Returns"] == pytest.approx(0.0096, rel=0.01)
        ...
        assert updated["TICKER2"].iloc[-1]["Price"] == pytest.approx(55.0)
        assert updated["TICKER2"].iloc[-1]["Returns"] == pytest.approx(0.0185, rel=0.01)
