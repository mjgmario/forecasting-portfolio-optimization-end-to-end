...
def preprocess_data(all_stock_data: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
    ...
    # Ensure all indexes are DatetimeIndex and find intersection of dates across all tickers
...
def append_predictions(
    ...
    Raises:
        ValueError: If ticker is missing from predictions or predicted_returns
    """
    updated_portfolio_data = {}

    for ticker, df in portfolio_data.items():
        # Validate ticker exists in predictions
        if ticker not in predictions or ticker not in predicted_returns:
            raise ValueError(f"Missing prediction data for ticker: {ticker}")

        # Skip empty DataFrames
        if df.empty:
            logger.warning(f"Skipping empty DataFrame for ticker: {ticker}")
            continue
